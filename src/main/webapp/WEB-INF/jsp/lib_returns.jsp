...
<%@ taglib prefix="simple" uri="date-tag.tld"%>
<html>
<head>
    <meta charset="utf-8">
...
<c:if test="${requestScope.bookstoconfirm.size() == 0}">
...
<table border="1" class="found-books">
    <caption>${returnedBooks}</caption>
    <th>${name}</th>
    <th>${user}</th>
    <th>${date}</th>
    <th>${action}</th>
    <c:forEach var="n" items="${requestScope.bookstoconfirm}" >
        <tr>
            <td>
            ...
            <td>
                <form action="Controller" method="post">
                    <br/>
                    <input type="hidden" name="command" value="confirmbookreturn"/>
                    <input type="hidden" name="returnid" value="${n.operationID}"/>
